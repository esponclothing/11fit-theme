...
(function() {
  function initLuxeDeal(wrapper) {
    ...
    let endTime;
    try {
      endTime = localStorage.getItem('luxe_timer_' + sectionId);
      if (!endTime || new Date().getTime() > endTime) {
        endTime = new Date().getTime() + (timerHours * 60 * 60 * 1000);
        localStorage.setItem('luxe_timer_' + sectionId, endTime);
      }
    } catch (e) {
      // Fallback if localStorage is blocked (e.g. incognito or Shopify Theme Editor iframe)
      endTime = new Date().getTime() + (timerHours * 60 * 60 * 1000);
    }
    ...
    function updateTimer() {
      ...
      if (distance < 0) {
        endTime = new Date().getTime() + (timerHours * 60 * 60 * 1000);
        try { localStorage.setItem('luxe_timer_' + sectionId, endTime); } catch (e) {}
