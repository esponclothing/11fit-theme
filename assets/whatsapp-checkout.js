  ...
  function waGetSavedPhone() {
    ...
    // 1. Shopify customer - authenticated by Shopify's own session
    const sc = window._shopifyCustomer;
    if (sc && sc.logged_in && sc.phone) {
      phone = sc.phone;
    }

    // 2. sessionStorage — ONLY written by successful verify-otp response
    if (!phone) {
      try {
        let v = sessionStorage.getItem('fit11_verified_phone');
    ...
    // 3. Cookie wa_saved_phone — ONLY written by successful verify-otp response (30-day)
    if (!phone) {
      try {
        const cookies = document.cookie.split('; ');
        for (let c of cookies) {
          let [name, val] = c.split('=');
          if (name === 'wa_saved_phone') {
  ...
  function waSwitchAccount(e) {
    ...
    // Generate a fresh device_id so identify no longer returns the old user's data
    const freshDeviceId = 'dev_' + Math.random().toString(36).substr(2, 9) + Date.now().toString(36);
    localStorage.setItem('fit11_device_id', freshDeviceId);
    localStorage.setItem('wa_device_id', freshDeviceId);
  ...
  async function calculateCheckoutTotals(discountCode = null) {
    ...
    try {
      ...
      if (!effectiveDiscountCode && cart.items && cart.items.length > 0) {
        ...
        for (const pid in prodCounts) {
          if (prodCounts[pid] >= 2) {
            effectiveDiscountCode = '11FIT-COMBO-' + pid + '-' + prodCounts[pid];
            break;
          }
        }
      }
...
function renderPaymentMethods() {
  ...
  async function waPayNow() {
      ...
      if (waSelectedPayment === 'cod') {
        // Strip any prepaid discount that may have been applied during this session before completing as COD
        try {
        ...
        } catch(e) { console.warn('update-draft for COD (non-fatal):', e); }
        // Direct complete
        ...
        if (netPayable <= 0) {
          ...
          const data = await res.json();
          if (!res.ok || !data) {
            throw new Error(data?.error || data?.message || 'Payment server failed to initiate payment.');
          }

          if (data.zero_amount) {
            // Order completed directly on server (covered 100% by credit/discount)
            document.getElementById('wa-step-4').style.display = 'none';
            ...
            const scBox = document.getElementById('wa-success-order-number');
            if (scVal && scBox && (data.order_id || data.shopify_order_id)) {
              const oNum = String(data.order_id || data.shopify_order_id);
